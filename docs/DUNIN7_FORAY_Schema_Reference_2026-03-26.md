...
**Status:** Confidential — Prior Conception Record. Not for public distribution.
...
| Parent Transaction ID | Lineage reference. Null if originating. Populated for all child transactions. |
...
> The Arrangement is the handshake — transaction type and parties only. It does not describe the deal. It declares that these parties are entering this type of transaction. All detail unfolds downstream.
...
> Accruals do not know about Arrangements. The Accrual references the Transaction ID. The Arrangement has no knowledge of what accrues from it.
...
| Expected Direction | To or from each identified party. |
...
> An Anticipation is not always a projection. Sometimes it is simply an awareness — the system recording that a condition exists and is being monitored.
...
| Actual Direction | To or from each identified party. |
| Actual Date/Time | Confirmed timestamp of execution. |
| Originating Anticipation ID | The Anticipation this Action resolves. An Action without a corresponding Anticipation in a transaction type that always has one is a structural anomaly. |
| Action Type | Transfer (asset moves between parties) or State Change (condition confirmed without asset movement). |

> The Action is the only universally mandatory component. Every valid FORAY transaction has at least one Action. Everything else is present only if the transaction requires it.
...
*Created: 2026-03-26 | Confidential — Not for public distribution*  
