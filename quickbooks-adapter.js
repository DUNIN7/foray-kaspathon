...
class QuickBooksForayAdapter {
  ...
  async anchorInvoice(invoice) {
    ...
    return await this._withRetry(async () => {
      ...
      this._log(`  &#x2714; Arrangement created: ${arrangement.id}`);
      ...
      this._log(`  &#x2714; Accrual created: ${accrual.id}`);
      ...
      this._log(`  &#x2714; Anticipation created: ${anticipation.id}`);
      ...
      if (invoice.Balance === 0 && invoice.TotalAmt > 0) {
        ...
        this._log(`  &#x2714; Action created: ${action.id} (PAID)`);
      }
      ...
      this._log(`  &#x2714; Anchored to Kaspa: ${anchorResult.kaspaHash}`);
