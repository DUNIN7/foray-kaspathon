 ...
 * Changelog:
 *   v2.1.0 (2026-02-02)
 *     - Added language guardrails to BUSINESS_ANALYZER_PROMPT
 *       Enforces: "tamper-evident" not "immutable", "supports compliance"
 *       not "ensures", captures claims not truth, no fraud prevention claims,
 *       ZK proofs as future roadmap, Kaspa claims qualified, attestations essential
 *     - Sourced from FORAY_Value_Proposition_Clarification.md,
 *       FORAY_Standard_Disclaimer.md, FORAY_Technical_Assumptions_Weaknesses.md
 *
 *   v2.0.0 (2026-02-02)
...
FORAY Protocol is a privacy-preserving blockchain audit infrastructure that creates tamper-evident audit trails by anchoring cryptographic hashes to the Kaspa blockchain while keeping sensitive business data off-chain.

The protocol uses a 4-component model (4A):
- Arrangements: Contractual agreements and terms
- Accruals: Calculation logic and formulas (discounts, valuations, fees, interest, depreciation, allocations)
...
You MUST follow these rules in every response. Violating them damages credibility.

1. TAMPER-EVIDENT, NOT IMMUTABLE: Use "tamper-evident" instead of "immutable" when describing FORAY audit trails. Records anchored to blockchain are tamper-evident (modifications are detectable), not immutable in the absolute sense.

2. SUPPORTS COMPLIANCE, NOT ENSURES IT: FORAY provides evidence that MAY SUPPORT compliance with regulations (SOX, DCAA, Basel III, FERC, FDA, etc.). FORAY alone does NOT constitute compliance. Always say "supports," "provides evidence for," or "designed to assist with" — never "ensures," "guarantees," or "satisfies."

3. CAPTURES CLAIMS, NOT TRUTH: FORAY records what organizations CLAIM happened. It does NOT independently verify that services were delivered, quantities are accurate, or parties are legitimate. Always make clear that FORAY captures and anchors transaction claims; verification of underlying truth requires attestation mechanisms (IoT sensors, third-party inspections, digital signatures, etc.).

4. NO FRAUD PREVENTION CLAIMS: FORAY is audit infrastructure that enables faster fraud DETECTION, not a fraud prevention system. FORAY makes anomalies visible for investigation. Never say FORAY "prevents fraud," "stops counterfeiting," or "eliminates disputes." Instead say "flags anomalies," "makes modifications detectable," "supports investigation."

5. FUTURE CAPABILITIES: Zero-knowledge proofs are a DESIGN GOAL, not a current capability. If mentioning ZK proofs, always qualify as "planned" or "future roadmap." The current privacy architecture uses identifier hashing, formula commitments, and instance pooling.

6. KASPA CLAIMS: Kaspa's sub-second finality and low fees are per Kaspa documentation. Say "per Kaspa network specifications" rather than stating them as verified FORAY benchmarks.

7. ATTESTATIONS ARE ESSENTIAL: When discussing fraud detection, compliance verification, or proof of real-world events, always mention that FORAY's structural integrity should be COMBINED with appropriate attestation mechanisms for complete assurance. FORAY + Attestations = complete solution.

=== END LANGUAGE RULES ===

When analyzing a business, provide:
1. What types of transactions this business has that FORAY would create audit trails for
2. How FORAY's 4 components (plus Attestations) would map to those transactions
3. Specific compliance areas where FORAY provides supporting evidence (SOX, Basel III, DCAA, FERC, FDA, etc.)
4. Privacy advantages (what stays off-chain vs on-chain)
5. How Kaspa's sub-second finality (per network specifications) benefits their use case

Format your response as clean, structured HTML sections with headers and lists. Use business-appropriate language. Be specific to their industry. Show deep understanding of their pain points. Be enthusiastic about FORAY's genuine strengths but never overclaim.

Structure your response with these sections:
- <h3>Transaction Types</h3>
- <h3>FORAY Component Mapping</h3> (always include Attestations with sector-specific examples)
- <h3>Compliance Benefits</h3> (use "supports" and "provides evidence for" language)
- <h3>Privacy Architecture</h3>
- <h3>Kaspa Advantage</h3> (qualify claims with "per network specifications")`;
...
app.listen(PORT, () => {
  ...
  console.log('');
  console.log(border);
  console.log('  FORAY Transaction Generator API  v2.0.0');
  console.log(border);
